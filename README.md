...
**2026年9月1号 至 2026年9月15号 商业版代码更新内容**

736、修复Binder拦截后异常类型发生变化，导致APP无法正确处理异常的问题<br/>
735、适配部分Android 16手机上Service连接回调的参数差异<br/>
734、修复账号token失效后清理不完整，仍可能获取到旧token的问题<br/>
733、修复普通账号认证成功后token未保存的问题<br/>
732、修复自定义账号token缓存命中后仍重复请求认证的问题<br/>
731、修复stopService返回状态错误，完善服务启动与停止衔接处理<br/>
730、修复调用finishAffinity后Activity结束状态不正确的问题<br/>
729、修复Seccomp-Bpf模式下使用相对路径添加inotify监听失败的问题<br/>
728、修复futimens更新文件时间时错误返回权限异常的问题<br/>
727、修复syscall(truncate)参数传递错误导致文件截断失败的问题<br/>
726、修复账号可见性配置在重启后丢失或归属到错误分身的问题<br/>
725、修复取消全部闹钟时误取消其他多开APP闹钟的问题<br/>
724、修复查询通知时混入其他APP通知，以及使用返回的tag取消通知失败的问题<br/>
723、修复查询通知监听权限和气泡配置时出现空指针的问题<br/>
722、修复多包安装完成后父安装会话未结束、完成通知缺失的问题<br/>
721、完善安装会话权限校验，防止不同APP操作彼此的安装任务<br/>
720、修复在非0虚拟用户中安装新APP时，错误安装到用户0的问题<br/>
719、修复安装包含重复APK的安装包时，安装线程卡死的问题<br/>
